  ...
  return (
    <div className="flex flex-col items-center w-full max-w-md mx-auto">
      ...
      <div className="mt-4 flex items-center gap-2">
        <p className="text-sm font-medium" style={{ color: '#6E5B52' }}>{statusLabel[status]}</p>
        <button
        ...
        {lastTurns.map((t, i) => (
          <div key={`${turns.length}-${i}`} className={`flex ${t.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              ...
              style={t.role === 'user'
                ? { background: '#181312' }
                : { background: '#FFFFFF', color: '#3A2A26', border: '1px solid #E5DCD2', boxShadow: '0 2px 12px rgba(24,19,18,0.05)' }}
            >
              {t.content || <Loader2 className="h-4 w-4 animate-spin" style={{ color: SKY }} />}
            </div>
          </div>
        ))}
        {interim && <p className="text-[13px] italic text-right" style={{ color: '#7A675D' }}>{interim}…</p>}
      </div>
      ...
      <div className="mt-5 flex flex-wrap justify-center gap-2">
        {CHIPS.map((c) => (
          <button
            ...
            style={{ background: '#FFFFFF', border: '1px solid #E5DCD2', color: '#5A463F' }}
          >
            {c}
          </button>
        ))}
      ...
      >
        <input
          ...
          style={{ background: '#FFFFFF', border: '1px solid #E5DCD2', color: '#181312' }}
        />
        <button
          type="submit"
