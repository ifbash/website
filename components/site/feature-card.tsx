...
export function FeatureCard({
    ...
    <>
      ...
      {points && points.length > 0 && (
        <ul className="mt-4 space-y-1.5">
          {points.map((p) => (
            <li key={p} className="flex items-start gap-2 text-[13px] text-slate">
  ...
  const shell = cn(
    ...
    'transition-all duration-300 hover:border-sea-soft hover:shadow-[0_8px_30px_rgba(11,20,23,0.06)] hover:-translate-y-0.5',
    className,
  );
