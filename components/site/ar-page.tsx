    ...
    <>
      <Section tone="paper" className="pt-14 md:pt-20 pb-14">
        <Container>
          <div className="max-w-3xl">
            <p className="text-[11px] font-semibold tracking-[0.15em] text-navy mb-5">{eyebrow}</p>
            <h1 className="font-arabic leading-[1.5] text-ink mb-6 text-[clamp(1.75rem,4.2vw,2.8rem)] font-semibold">
              {headline} <span className="text-navy">{accent}</span>
            </h1>
            <p className="text-[16.5px] leading-[1.9] text-stone mb-8 max-w-2xl">{sub}</p>

            <div className="flex flex-wrap items-center gap-3">
              <Link
                href="/ar/get-started"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-[14.5px] font-semibold bg-ink text-paper hover:bg-navy transition-colors"
              >
              ...
              <Link
                href="/ar/engage"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-[14.5px] font-medium border border-hairline bg-surface text-ink-body hover:border-navy-strong hover:text-navy transition-colors"
              >
      ...
      {items && items.length > 0 && (
        <Section tone="surface" divide>
          <Container>
            {itemsEyebrow && (
              <p className="text-[11px] font-semibold tracking-[0.15em] text-navy mb-8">
                {itemsEyebrow}
              </p>
            )}
            <div className="grid md:grid-cols-2 gap-5">
              {items.map((it) => (
                <div key={it.title} className="rounded-2xl border border-hairline bg-paper p-6">
                  ...
                  <p className="text-[14.5px] leading-[1.85] text-stone">{it.desc}</p>
                </div>
              ))}
            </div>
          </Container>
      ...
      {note && (
        <Section tone="wash">
          <Container>
            <p className="text-[14.5px] leading-[1.9] text-stone max-w-2xl border-r-2 border-navy-soft pr-5">
              {note}
            </p>
          </Container>
        </Section>
        ...
        <Container>
          <div className="max-w-2xl">
            <h2 className="font-arabic text-[clamp(1.5rem,3vw,2.1rem)] font-semibold leading-[1.5] text-onink mb-5">
              {ar.home.ctaHeadline} <span className="text-navy-soft">{ar.home.ctaAccent}</span>
            </h2>
