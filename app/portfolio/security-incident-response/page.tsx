...
const capabilities = [
  { icon: Shield, title: 'Workflow Management', desc: 'Automated assignment, intelligent prioritization, and cross-team orchestration. Every incident follows the right path.', outcomes: ['Automated assignment', 'Intelligent prioritization', 'Cross-team orchestration'] },
  { icon: Eye, title: 'Operations Dashboard', desc: 'Real-time SOC performance visibility. Know where to evolve your team and workflows for maximum impact.', outcomes: ['Real-time visibility', 'Performance metrics', 'Evolution insights'] },
  { icon: Target, title: 'MITRE ATT&CK Framework', desc: 'Tactic and technique mapping, advanced threat context, and threat hunting enhancement built into every incident.', outcomes: ['TTP mapping', 'Threat context', 'Hunting enhancement'] },
  { icon: Cog, title: 'Major Incident Management', desc: 'Dedicated workspace for ransomware, data breaches, and critical threats. Coordinated response across every team.', outcomes: ['Ransomware response', 'Data breach management', 'Crisis coordination'] },
];

const methodology = [
  { phase: 'Assess', duration: 'Weeks 1–3', icon: Search, items: ['Incident response maturity audit', 'Playbook & workflow mapping', 'Tool integration baseline'] },
  { phase: 'Build', duration: 'Weeks 4–14', icon: Shield, items: ['SIR platform configuration', 'MITRE ATT&CK mapping & integration', 'Playbook automation & testing'] },
  { phase: 'Launch', duration: 'Weeks 15–20', icon: Rocket, items: ['SOC team training & simulation', 'Phased go-live & monitoring', 'KPI baseline establishment'] },
  { phase: 'Optimize', duration: 'Weeks 21–26', icon: TrendingUp, items: ['Performance analytics & tuning', 'Playbook refinement & expansion', 'Quarterly maturity assessment'] },
];

const caseStudies = [
  { client: 'Fortune 500 Technology Company', industry: 'Technology', timeline: '20 weeks', challenge: 'Manual incident response processes. Analysts overwhelmed by alert volume. Response times measured in days, not hours.', solution: 'ServiceNow SIR with MITRE ATT&CK integration and automated response playbooks across the global SOC.', results: ['6x faster incident processing', 'Automated playbooks deployed', 'MITRE ATT&CK fully mapped', 'SOC efficiency transformed'], quote: 'Six times faster incident processing. Our analysts finally focus on hunting threats instead of pushing paper.', author: 'Chief Information Security Officer' },
];
...
const faqs = [
  ...
  { q: 'How long does implementation take?', a: '12–22 weeks depending on SOC maturity. Platform and playbooks deploy in parallel. MITRE ATT&CK mapping active from day one.' },
    ...
    <>
      ...
      <section className="relative bg-white overflow-hidden flex items-center min-h-[calc(100vh-70px)]">
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto py-12 sm:py-14">
          <div className="max-w-3xl">
            ...
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold text-gray-900 leading-[1.06] tracking-tight mb-6">Every threat.<br /><span style={{ color: A }}>Contained and resolved.</span></h1>
            <p className="text-lg text-gray-500 leading-relaxed mb-10 max-w-xl">Respond rapidly to evolving threats with MITRE ATT&CK integration and AI-powered incident response. <span className="text-gray-900 font-semibold">6x faster incident processing. Complete SOC visibility.</span></p>
      ...
      <section className="py-16 lg:py-20 relative overflow-hidden" style={{ background: '#fef2f2' }}>
        ...
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto relative z-10">
          ...
          <div data-reveal="capabilities" className={`grid sm:grid-cols-2 lg:grid-cols-2 gap-5 transition-all duration-700 ${revealed.capabilities ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
