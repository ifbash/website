...
import { ArrowRight, CheckCircle, ChevronDown, ChevronLeft, ChevronRight, MessageCircle, Search, Lightbulb, Cog, Rocket, Building, Target, Layers, TrendingUp, Clock, Shield, ClipboardCheck } from 'lucide-react';

const A = '#4f46e5';
const AG = 'linear-gradient(135deg, #4f46e5, #7c3aed)';

const capabilities = [
  { icon: Building, title: 'Application Rationalization', desc: 'Data-driven invest/sustain/retire decisions for every application. Eliminate redundancies and cut maintenance costs.', outcomes: ['Invest/sustain/retire', 'Redundancy elimination', 'TCO analysis'] },
  { icon: Target, title: 'Business Capability Management', desc: 'Remove process and technology gaps. Align capabilities to strategy and track maturity over time.', outcomes: ['Gap elimination', 'Strategy alignment', 'Maturity tracking'] },
  { icon: Layers, title: 'Technology Portfolio Management', desc: 'Lifecycle monitoring, risk assessment, and technology standards enforcement across your entire portfolio.', outcomes: ['Lifecycle monitoring', 'Risk assessment', 'Standards enforcement'] },
  { icon: TrendingUp, title: 'Enterprise Modeling', desc: 'CSDM-aligned visualization of your entire tech estate. Relationship mapping and impact analysis in real time.', outcomes: ['CSDM-aligned', 'Relationship mapping', 'Impact analysis'] },
  { icon: Search, title: 'Digital Integration Management', desc: 'API landscape visualization, integration health monitoring, and dependency tracking across all systems.', outcomes: ['API landscape view', 'Health monitoring', 'Dependency tracking'] },
];

const methodology = [
  { phase: 'Assess', duration: 'Weeks 1–3', icon: Search, items: ['Application inventory & capability mapping', 'Redundancy & overlap analysis', 'Cost baseline & TCO modeling'] },
  { phase: 'Design', duration: 'Weeks 4–13', icon: Lightbulb, items: ['Target architecture definition', 'Rationalization roadmap', 'Technology standards & guardrails'] },
  { phase: 'Transform', duration: 'Weeks 14–20', icon: Cog, items: ['Migration execution & tracking', 'Application retirement & consolidation', 'Capability realignment & validation'] },
];

const caseStudies = [
  { client: 'IBM', industry: 'Technology Services', timeline: '18 weeks', challenge: 'Fragmented application portfolio across global operations. Architecture decisions made without data, leading to costly overlaps.', solution: 'ServiceNow Enterprise Architecture with Application Portfolio Management across all business units.', results: ['Unified global visibility', 'Faster architecture decisions', 'Streamlined operations', 'Reduced portfolio costs'], quote: 'ServiceNow EA gave us the visibility to make data-driven architecture decisions instead of relying on spreadsheets and opinions.', author: 'Alison Sorbet, Operations Manager' },
  { client: 'Fortune 500 Manufacturer', industry: 'Manufacturing', timeline: '16 weeks', challenge: 'Application sprawl across divisions. Rising maintenance costs. No visibility into what each application actually did for the business.', solution: 'ServiceNow EA with Application Rationalization and Business Capability Management.', results: ['35% cost reduction', '50% faster delivery', '200+ redundant apps identified', 'Clear investment roadmap'], quote: 'We found over 200 redundant applications. The savings were immediate and the clarity was transformative.', author: 'Enterprise Architect Lead' },
];
...
const faqs = [
  ...
  { q: 'How long does implementation take?', a: '10–20 weeks depending on portfolio size. Assessment and initial findings delivered in weeks 1–3. Rationalization roadmap follows.' },
    ...
    <>
      ...
      <section className="relative bg-white overflow-hidden flex items-center min-h-[calc(100vh-70px)]">
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto py-12 sm:py-14">
          <div className="max-w-3xl">
            <div className="flex items-center gap-3 mb-8"><span className="inline-block w-8 h-px bg-indigo-600" /><span className="text-indigo-600 text-sm font-semibold tracking-widest uppercase">IT & Digital Operations</span></div>
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold text-gray-900 leading-[1.06] tracking-tight mb-6">See your entire tech estate.<br /><span className="text-indigo-600">Make every dollar count.</span></h1>
            <p className="text-lg text-gray-500 leading-relaxed mb-10 max-w-xl">Data-driven insights to assess, monitor, and align your application portfolio to business capabilities. <span className="text-gray-900 font-semibold">30% average cost reduction.</span> No more spreadsheets. No more opinions. Just data.</p>
      ...
      <section className="py-16 lg:py-20 bg-white">
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
          <div className="text-center mb-12"><h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-3">How We Deliver</h2><p className="text-gray-500 max-w-xl mx-auto">Assessment to transformation in three phases.</p></div>
          <div className="grid md:grid-cols-3 gap-6 relative max-w-3xl mx-auto">
      ...
      <section className="py-16 lg:py-20 relative overflow-hidden" style={{ background: '#07071a' }}>
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto"><div className="max-w-4xl mx-auto">
          <div className="text-center mb-12"><div className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-bold uppercase tracking-widest mb-8 border" style={{ backgroundColor: `${A}08`, borderColor: `${A}33`, color: '#818cf8' }}>Why ifBash</div><h2 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.06] tracking-tight mb-6">We don&apos;t just model.<br /><span style={{ color: '#818cf8' }}>We make architecture actionable.</span></h2><p className="text-slate-400 text-base lg:text-lg max-w-2xl mx-auto leading-relaxed mb-12">We start with your business capabilities and your P&L, not the platform.</p></div>
          <div data-reveal="whyifbash" className={`grid sm:grid-cols-3 gap-5 max-w-3xl mx-auto transition-all duration-700 ${revealed.whyifbash ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            {[{ icon: ClipboardCheck, stat: 'Process first', desc: 'We map your business capabilities before analyzing your application portfolio. Strategy drives architecture.' },{ icon: Clock, stat: 'Fast insights', desc: 'Initial findings in weeks 1–3. Rationalization roadmap delivered within the first month.' },{ icon: Shield, stat: 'We stay', desc: '90-day hypercare minimum. Architecture governance keeps your portfolio aligned as you grow.' }].map(({ icon: Icon, stat, desc }) => (<div key={stat} className="rounded-2xl border p-6 text-center hover:border-indigo-500/30 transition-all" style={{ borderColor: `${A}26`, background: `${A}0A` }}><div className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-4" style={{ background: `${A}18` }}><Icon className="h-5 w-5 text-indigo-400" /></div><div className="text-sm font-bold text-white mb-2">{stat}</div><p className="text-xs text-slate-400 leading-relaxed">{desc}</p></div>))}</div>
        </div></div>
      </section>
      ...
      <section className="py-16 relative overflow-hidden" style={{ background: '#07071a' }}>
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto text-center">
          <div className="max-w-xl mx-auto rounded-3xl border p-8 sm:p-10 text-center" style={{ borderColor: `${A}4d`, background: `linear-gradient(135deg, ${A}14, ${A}07)` }}>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-white leading-tight mb-3">Ready to rationalize your technology portfolio?</h2>
            <p className="text-slate-400 text-sm max-w-sm mx-auto mb-6">Tell us about your application landscape. We&apos;ll scope your EA engagement in 48 hours.</p>
