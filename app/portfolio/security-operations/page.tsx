...
import { ArrowRight, CheckCircle, ChevronDown, ChevronLeft, ChevronRight, MessageCircle, Search, Cog, Rocket, TrendingUp, Shield, Eye, Target, Clock, ClipboardCheck } from 'lucide-react';

const A = '#dc2626';
const AG = 'linear-gradient(135deg, #dc2626, #ef4444)';

const capabilities = [
  { icon: Shield, title: 'Security Incident Response', desc: 'Prioritize and respond to threats with intelligent workflows and MITRE ATT&CK integration for accelerated investigation.', outcomes: ['Intelligent prioritization', 'MITRE ATT&CK mapped', 'Automated workflows'] },
  { icon: Target, title: 'Vulnerability Response', desc: 'Risk-based vulnerability management across your entire infrastructure. Prioritize by business impact, not just CVSS score.', outcomes: ['Risk-based prioritization', 'Business impact context', 'Automated remediation'] },
  { icon: Eye, title: 'Security Posture Control', desc: '360° visibility into your attack surface. Detect security control gaps and unmanaged assets before attackers do.', outcomes: ['360° visibility', 'Control gap detection', 'Unmanaged asset alerts'] },
  { icon: TrendingUp, title: 'Threat Intelligence', desc: 'Advanced threat hunting, modeling, and analysis. Correlate intelligence across your entire security ecosystem.', outcomes: ['Threat hunting', 'Intelligence correlation', 'IOC management'] },
  { icon: Cog, title: 'Performance Analytics', desc: 'Real-time security metrics, trend analysis, and resource prioritization. Know your security posture at every moment.', outcomes: ['Real-time metrics', 'Trend analysis', 'Resource optimization'] },
];

const methodology = [
  { phase: 'Assess', duration: 'Weeks 1–4', icon: Search, items: ['Security tool audit & integration plan', 'Threat landscape & risk assessment', 'Response workflow & playbook mapping'] },
  { phase: 'Build', duration: 'Weeks 5–16', icon: Shield, items: ['Platform configuration & IR setup', 'Vulnerability management deployment', 'SIEM & security tool integration'] },
  { phase: 'Automate', duration: 'Weeks 17–22', icon: Cog, items: ['Playbook automation & testing', 'AI/ML model configuration & tuning', 'Team training & incident simulation'] },
  { phase: 'Optimize', duration: 'Weeks 23–28', icon: TrendingUp, items: ['Performance analytics & tuning', 'Continuous improvement cycles', 'Quarterly security posture reviews'] },
];

const caseStudies = [
  { client: 'Wellstar Health System', industry: 'Healthcare', timeline: '22 weeks', challenge: 'Protecting patient data and critical healthcare systems while maintaining operational efficiency and regulatory compliance across 11 hospitals.', solution: 'ServiceNow Security Operations with incident response, vulnerability management, and healthcare-specific security controls.', results: ['Enhanced patient data protection', 'Improved security compliance', 'Streamlined incident response', 'Operational confidence restored'], quote: 'ServiceNow SecOps enables us to deliver patient care with quality and confidence by protecting our healthcare systems around the clock.', author: 'Chief Information Security Officer' },
  { client: 'SAS Institute', industry: 'Analytics Software', timeline: '18 weeks', challenge: 'Complex threat landscape requiring faster incident response. Security team spending more time coordinating than investigating.', solution: 'ServiceNow Security Operations with advanced incident response, threat intelligence integration, and automated orchestration.', results: ['Strengthened incident management', 'Faster threat response', 'Enhanced security automation', 'Improved global coordination'], quote: 'ServiceNow SecOps strengthened our incident management capabilities, enabling significantly faster threat response across our global operations.', author: 'VP of Information Security' },
];
...
const faqs = [
  ...
  { q: 'How long does implementation take?', a: '14–28 weeks depending on scope. Security tool audit in weeks 1–4. Incident response and vulnerability management configured in parallel. Playbooks deploy continuously.' },
  ...
  { q: 'How does it improve SOC efficiency?', a: 'Automated playbooks handle routine tasks. AI triages alerts by business impact. Analysts focus on real threats instead of chasing false positives. Most SOCs see 60%+ efficiency gains within the first quarter.' },
];
...
export default function SecurityOperationsPage() {
  ...
  return (
    <>
      ...
      <section className="relative bg-white overflow-hidden flex items-center min-h-[calc(100vh-70px)]">
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto py-12 sm:py-14">
          <div className="max-w-3xl">
            <div className="flex items-center gap-3 mb-8"><span className="inline-block w-8 h-px" style={{ backgroundColor: A }} /><span className="text-sm font-semibold tracking-widest uppercase" style={{ color: A }}>Risk & Security</span></div>
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold text-gray-900 leading-[1.06] tracking-tight mb-6">Simplify threats.<br /><span style={{ color: A }}>Automate response.</span></h1>
            <p className="text-lg text-gray-500 leading-relaxed mb-10 max-w-xl">AI-powered security operations that connect your tools, automate workflows, and prioritize what matters. <span className="text-gray-900 font-semibold">Faster response. Lower risk. Complete visibility.</span></p>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Link href="/company/case-studies-client-success" className="inline-flex items-center justify-center gap-2 px-7 py-3.5 border border-gray-200 hover:border-red-400 text-gray-600 hover:text-red-600 font-semibold rounded-xl transition-colors text-sm">See Client Results</Link>
            </div>
          </div>
        </div>
      </section>
      <section className="py-16 lg:py-20 relative overflow-hidden" style={{ background: '#fef2f2' }}>
        ...
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto relative z-10">
          ...
          <div data-reveal="capabilities" className={`grid sm:grid-cols-2 lg:grid-cols-3 gap-5 transition-all duration-700 ${revealed.capabilities ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            {capabilities.map(({ icon: Icon, title, desc, outcomes }, i) => (
              <div key={title} className="group p-6 rounded-2xl border border-red-200 bg-white transition-all hover:shadow-md" style={{ transitionDelay: `${i * 80}ms` }}>
          ...
          <div className="grid md:grid-cols-4 gap-6 relative">
            ...
            {methodology.map(({ phase, duration, icon: Icon, items }) => (
              <div key={phase} className="relative z-10 group text-center">
                <div className="w-14 h-14 mx-auto mb-3 rounded-2xl bg-white border-2 border-red-100 group-hover:border-red-300 flex items-center justify-center transition-all shadow-sm group-hover:shadow-md"><Icon className="h-6 w-6" style={{ color: A }} /></div>
      ...
      <section className="py-16 lg:py-20" style={{ background: '#fef2f2' }}>
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
      ...
      <section className="py-16 lg:py-20 relative overflow-hidden" style={{ background: '#07071a' }}>
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto"><div className="max-w-4xl mx-auto">
          <div className="text-center mb-12"><div className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-bold uppercase tracking-widest mb-8 border" style={{ backgroundColor: `${A}08`, borderColor: `${A}33`, color: A }}>Why ifBash</div><h2 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.06] tracking-tight mb-6">We don&apos;t just secure systems.<br /><span style={{ color: A }}>We eliminate the noise.</span></h2><p className="text-slate-400 text-base lg:text-lg max-w-2xl mx-auto leading-relaxed mb-12">We start with your security team and their alert fatigue, not the platform.</p></div>
          <div data-reveal="whyifbash" className={`grid sm:grid-cols-3 gap-5 max-w-3xl mx-auto transition-all duration-700 ${revealed.whyifbash ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            {[{ icon: ClipboardCheck, stat: 'Process first', desc: 'We audit your security stack before writing a single playbook. Every automation targets a real analyst pain point.' },{ icon: Clock, stat: 'Speed + quality', desc: 'Incident response playbooks deploying in weeks. MITRE ATT&CK mapping from day one.' },{ icon: Shield, stat: 'We stay', desc: '90-day hypercare minimum. We tune playbooks until your SOC runs like a well-oiled machine.' }].map(({ icon: Icon, stat, desc }) => (<div key={stat} className="rounded-2xl border p-6 text-center transition-all" style={{ borderColor: `${A}26`, background: `${A}0A` }}><div className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-4" style={{ background: `${A}18` }}><Icon className="h-5 w-5" style={{ color: A }} /></div><div className="text-sm font-bold text-white mb-2">{stat}</div><p className="text-xs text-slate-400 leading-relaxed">{desc}</p></div>))}</div>
        </div></div>
      </section>
      <section className="py-16 lg:py-20 bg-white">
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
      ...
      <section className="py-16 relative overflow-hidden" style={{ background: '#07071a' }}>
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto text-center">
          <div className="max-w-xl mx-auto rounded-3xl border p-8 sm:p-10 text-center" style={{ borderColor: `${A}4d`, background: `linear-gradient(135deg, ${A}14, ${A}07)` }}>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-white leading-tight mb-3">Ready to transform your security operations?</h2>
            <p className="text-slate-400 text-sm max-w-sm mx-auto mb-6">Tell us about your security stack. We&apos;ll scope your SecOps implementation in 48 hours.</p>
