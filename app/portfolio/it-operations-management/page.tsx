...
import { ArrowRight, CheckCircle, ChevronDown, ChevronLeft, ChevronRight, MessageCircle, Search, Cog, TrendingUp, Monitor, Cloud, Eye, BarChart3, Clock, Shield, ClipboardCheck } from 'lucide-react';

const A = '#4f46e5';
const AG = 'linear-gradient(135deg, #4f46e5, #7c3aed)';

const capabilities = [
  { icon: Eye, title: 'Discovery', desc: 'Holistic view across on-prem data centers and cloud. Automated infrastructure inventory, dependency mapping, and real-time updates.', outcomes: ['Complete visibility', 'Auto-discovery', 'Dependency maps'] },
  { icon: Monitor, title: 'Service Mapping', desc: 'Map IT components to business services in dynamic environments. Visualize the impact of change before it happens.', outcomes: ['Real-time maps', 'Impact visualization', 'Dynamic updates'] },
  { icon: BarChart3, title: 'Event Management', desc: 'Reduce event floods from monitoring tools. Intelligent correlation surfaces what matters, not what makes noise.', outcomes: ['99% noise reduction', 'Intelligent correlation', 'Service health view'] },
  { icon: Cloud, title: 'AIOps & Agentic Workflows', desc: 'Generative AI triages alerts faster and more accurately. Autonomous resolution for known issues — your team focuses on what matters.', outcomes: ['99% fewer false positives', 'GenAI triage', 'Auto-resolution'] },
  { icon: TrendingUp, title: 'Cloud Accelerate', desc: 'Automated cloud service delivery with continuous governance. Speed cloud adoption without sacrificing control.', outcomes: ['60% faster adoption', 'Continuous governance', 'Multi-cloud support'] },
];

const methodology = [
  { phase: 'Discover', duration: 'Weeks 1–5', icon: Search, items: ['Environment discovery & infrastructure mapping', 'Dependency analysis & service topology', 'Event source inventory & baselining'] },
  { phase: 'Configure', duration: 'Weeks 6–17', icon: Cog, items: ['Platform setup & Service Mapping', 'Event correlation rules & topology', 'Cloud connector configuration'] },
  { phase: 'Automate', duration: 'Weeks 18–23', icon: Cloud, items: ['AIOps configuration & tuning', 'Agentic workflow deployment', 'Alert correlation validation'] },
  { phase: 'Optimize', duration: 'Weeks 24–28', icon: TrendingUp, items: ['Dashboard creation & KPI framework', 'Team training & knowledge transfer', 'Continuous improvement cycle'] },
];

const caseStudies = [
  { client: 'NBA', industry: 'Sports & Entertainment', timeline: '22 weeks', challenge: '200 million daily logs generating overwhelming alert noise. Operations team drowning in false positives and unable to identify real issues.', solution: 'ServiceNow ITOM AIOps with automated event management and intelligent correlation across all monitoring sources.', results: ['99.999% alert reduction', '200M logs processed daily', 'Proactive operations enabled', 'Team focus restored'], quote: 'Reduced alerts from overwhelming to actionable. Our team can finally focus on what matters instead of chasing ghosts.', author: 'Mark Bennett, SVP' },
  { client: 'Munich Re', industry: 'Financial Services', timeline: '20 weeks', challenge: 'No end-to-end CMDB visibility across global operations. Audit preparation was manual, time-consuming, and incomplete.', solution: 'ServiceNow ITOM Visibility with Service Mapping and comprehensive CMDB population across all regions.', results: ['Complete CMDB visibility', 'Enhanced audit capability', 'Faster incident resolution', 'Global service mapping'], quote: 'End-to-end visibility transformed our audit and incident response. What took weeks of manual work now happens automatically.', author: 'IT Operations Director' },
];
...
const faqs = [
  ...
  { q: 'How does AIOps reduce false positives?', a: 'Generative AI analyzes telemetry and log data, correlates related events, and eliminates up to 99% of noise. Your team focuses on real issues instead of chasing false alarms.' },
  ...
  { q: 'How long does implementation take?', a: '18–28 weeks depending on scope. Discovery and service mapping deploy first (weeks 1–5). AIOps and automation follow with continuous refinement.' },
    ...
    <>
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto py-12 sm:py-14">
          <div className="max-w-3xl">
            ...
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold text-gray-900 leading-[1.06] tracking-tight mb-6">
              See everything.
              <br />
              <span className="text-indigo-600">Predict everything.</span>
            </h1>
            <p className="text-lg text-gray-500 leading-relaxed mb-10 max-w-xl">
              Gain full visibility across on-premises and cloud. AIOps eliminates 99% of false positives. <span className="text-gray-900 font-semibold">60% faster cloud adoption</span> with automated governance.
            </p>
      ...
      <section className="py-16 lg:py-20 bg-white">
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
          <div className="text-center mb-12"><h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-3">How We Deliver</h2><p className="text-gray-500 max-w-xl mx-auto">Proven methodology. Predictable outcomes. Visibility from week one.</p></div>
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h2 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.06] tracking-tight mb-6">We don&apos;t just monitor.<br /><span style={{ color: '#818cf8' }}>We make sense of it all.</span></h2>
              <p className="text-slate-400 text-base lg:text-lg max-w-2xl mx-auto leading-relaxed mb-12">We start with your infrastructure and your outcomes, not the platform.</p>
            </div>
            <div data-reveal="whyifbash" className={`grid sm:grid-cols-3 gap-5 max-w-3xl mx-auto transition-all duration-700 ${revealed.whyifbash ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
              {[
                { icon: ClipboardCheck, stat: 'Process first', desc: 'We map your entire IT estate before touching a single configuration. Every alert tuned to your reality.' },
                { icon: Clock, stat: 'Speed + quality', desc: 'Service Mapping and event correlation deploy in parallel. Visibility from week one.' },
                { icon: Shield, stat: 'We stay', desc: '90-day hypercare minimum. We tune AIOps models until they understand your environment perfectly.' },
      ...
      <section className="py-16 relative overflow-hidden" style={{ background: '#07071a' }}>
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto text-center">
          <div className="max-w-xl mx-auto rounded-3xl border p-8 sm:p-10 text-center" style={{ borderColor: `${A}4d`, background: `linear-gradient(135deg, ${A}14, ${A}07)` }}>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-white leading-tight mb-3">Ready to see your entire IT estate?</h2>
            <p className="text-slate-400 text-sm max-w-sm mx-auto mb-6">Tell us about your environment. We&apos;ll scope your ITOM implementation in 48 hours.</p>
