...
import { ArrowRight, CheckCircle, ChevronDown, ChevronLeft, ChevronRight, MessageCircle, Search, Cog, Rocket, TrendingUp, Users, Clock, Shield, ClipboardCheck, BarChart3 } from 'lucide-react';

const A = '#d97706';
const AG = 'linear-gradient(135deg, #d97706, #f59e0b)';

const capabilities = [
  { icon: Users, title: 'Employee Center', desc: 'Single unified portal for all employee services. HR, IT, facilities — one place, one experience, one login.', outcomes: ['Unified portal', 'Cross-department', 'Personalized'] },
  { icon: TrendingUp, title: 'Employee Journey Management', desc: 'Complete workflow automation for onboarding, transfers, relocations, and offboarding. No-code journey creation.', outcomes: ['No-code journeys', 'Lifecycle automation', 'Complete visibility'] },
  { icon: Cog, title: 'Case & Knowledge Management', desc: 'Standardized documentation, intelligent routing, and AI-powered knowledge suggestions. Answers before tickets.', outcomes: ['Standardized docs', 'Intelligent routing', 'AI knowledge'] },
  { icon: MessageCircle, title: 'Virtual Agent', desc: '24/7 AI-powered support with natural language understanding. Resolve HR issues fast without human intervention.', outcomes: ['24/7 availability', 'Natural language', 'Auto-resolution'] },
  { icon: BarChart3, title: 'Performance Analytics', desc: 'Real-time HR metrics, trend prediction, and resource optimization. Data-driven people decisions.', outcomes: ['Real-time metrics', 'Trend prediction', 'Resource insights'] },
];

const methodology = [
  { phase: 'Assess', duration: 'Weeks 1–3', icon: Search, items: ['Employee journey mapping & pain points', 'HR process audit & integration plan', 'Success criteria & KPI baseline'] },
  { phase: 'Build', duration: 'Weeks 4–12', icon: Cog, items: ['Employee Center configuration', 'Case & knowledge management setup', 'Virtual Agent training & deployment'] },
  { phase: 'Launch', duration: 'Weeks 13–18', icon: Rocket, items: ['HR team training & enablement', 'Phased employee rollout', 'KPI monitoring & adjustment'] },
  { phase: 'Optimize', duration: 'Weeks 19–24', icon: TrendingUp, items: ['Journey refinement & personalization', 'Self-service adoption campaigns', 'Continuous improvement cycles'] },
];

const caseStudies = [
  { client: 'Mondelez International', industry: 'Consumer Goods', timeline: '20 weeks', challenge: 'Fragmented HR service delivery across global operations. Low self-service adoption. HR teams buried in repetitive requests.', solution: 'ServiceNow HRSD with Employee Center, Virtual Agent, and Employee Journey Management across all regions.', results: ['76% self-service increase', 'Employee experience enhanced', 'Global operations unified', 'HR workload reduced 50%'], quote: 'A remarkable 76% increase in self-service. Our HR teams finally focus on strategic work instead of answering the same questions repeatedly.', author: 'Chief Human Resources Officer' },
  { client: 'Expedia Group', industry: 'Travel Technology', timeline: '18 weeks', challenge: 'Diverse global workforce with no unified employee portal. Every HR process required a different system or manual step.', solution: 'ServiceNow HRSD with unified Employee Center and journey management across the entire employee lifecycle.', results: ['Complete employee journey support', 'Unified portal deployed', 'Global workforce connected', 'Employee experience enhanced'], quote: 'One portal for every employee, everywhere. Our diverse workforce finally has a unified experience.', author: 'VP of Employee Experience' },
];
...
const faqs = [
  ...
  { q: 'How long does implementation take?', a: '8–24 weeks depending on scope. Employee Center and core case management deploy first (weeks 1–12). Journey management and analytics follow in later phases.' },
  ...
  { q: 'What ROI can we expect?', a: 'Clients typically see 50% reduction in HR workload, 70%+ self-service adoption, and ROI within 6–12 months. Mondelez saw a 76% increase in self-service alone.' },
];
...
export default function HRServiceDeliveryPage() {
  ...
  return (
    <>
      ...
      <section className="relative bg-white overflow-hidden flex items-center min-h-[calc(100vh-70px)]">
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto py-12 sm:py-14">
          <div className="max-w-3xl">
            <div className="flex items-center gap-3 mb-8"><span className="inline-block w-8 h-px" style={{ backgroundColor: A }} /><span className="text-sm font-semibold tracking-widest uppercase" style={{ color: A }}>HR & Workforce</span></div>
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold text-gray-900 leading-[1.06] tracking-tight mb-6">Every employee.<br /><span style={{ color: A }}>One place for everything.</span></h1>
            <p className="text-lg text-gray-500 leading-relaxed mb-10 max-w-xl">Improve productivity with AI-driven self-service and streamlined HR case resolution. <span className="text-gray-900 font-semibold">50% reduction in HR workload. 76% self-service increase.</span></p>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Link href="/company/case-studies-client-success" className="inline-flex items-center justify-center gap-2 px-7 py-3.5 border border-gray-200 hover:border-amber-400 text-gray-600 hover:text-amber-600 font-semibold rounded-xl transition-colors text-sm">See Client Results</Link>
            </div>
          </div>
        </div>
      </section>
      <section className="py-16 lg:py-20 relative overflow-hidden" style={{ background: '#fffbeb' }}>
        ...
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto relative z-10">
          ...
          <div data-reveal="capabilities" className={`grid sm:grid-cols-2 lg:grid-cols-3 gap-5 transition-all duration-700 ${revealed.capabilities ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            {capabilities.map(({ icon: Icon, title, desc, outcomes }, i) => (
              <div key={title} className="group p-6 rounded-2xl border border-amber-200 bg-white transition-all hover:shadow-md" style={{ transitionDelay: `${i * 80}ms` }}>
      ...
      <section className="py-16 lg:py-20 bg-white">
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
          ...
          <div className="grid md:grid-cols-4 gap-6 relative">
            <div className="hidden md:block absolute top-10 left-0 w-full h-px bg-gradient-to-r from-transparent via-amber-200 to-transparent z-0" />
            {methodology.map(({ phase, duration, icon: Icon, items }) => (
              <div key={phase} className="relative z-10 group text-center">
                <div className="w-14 h-14 mx-auto mb-3 rounded-2xl bg-white border-2 border-amber-100 group-hover:border-amber-300 flex items-center justify-center transition-all shadow-sm group-hover:shadow-md"><Icon className="h-6 w-6" style={{ color: A }} /></div>
      ...
      <section className="py-16 lg:py-20" style={{ background: '#fffbeb' }}>
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
