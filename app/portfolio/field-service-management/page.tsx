...
import { ArrowRight, CheckCircle, ChevronDown, ChevronLeft, ChevronRight, MessageCircle, Search, Cog, TrendingUp, Rocket, Wrench, Monitor, MapPin, Clock, Shield, ClipboardCheck } from 'lucide-react';

const A = '#0891b2';
const AG = 'linear-gradient(135deg, #0891b2, #06b6d4)';

const capabilities = [
  { icon: TrendingUp, title: 'Dynamic Scheduling', desc: 'Skill-based auto-assignment, location optimization, real-time schedule adjustment. The right tech, every time.', outcomes: ['70% efficiency', 'Auto-assignment', 'Real-time adjustment'] },
  { icon: Monitor, title: 'Mobile Agent', desc: 'Native iOS/Android with full offline capabilities. Everything a technician needs — in their pocket.', outcomes: ['Native mobile', 'Offline capable', 'Full FSM'] },
  { icon: MapPin, title: 'Schedule Optimization', desc: 'AI-powered route optimization across your entire fleet. Minimize travel, maximize jobs per day.', outcomes: ['40% less travel', 'AI routing', 'More jobs/day'] },
  { icon: Cog, title: 'Preventive Maintenance', desc: 'Automated maintenance scheduling and issue prediction. Fix problems before customers notice.', outcomes: ['85% prevention', 'Auto-scheduling', 'Issue prediction'] },
  { icon: Wrench, title: 'Dispatcher Workspace', desc: 'Unified view of all field operations. Real-time tracking, drag-and-drop scheduling, instant communication.', outcomes: ['Unified view', 'Real-time tracking', 'Drag-and-drop'] },
];

const methodology = [
  { phase: 'Assess', duration: 'Weeks 1–4', icon: Search, items: ['Field operations audit & resource analysis', 'Technology baseline & integration inventory', 'KPI baseline & success criteria'] },
  { phase: 'Configure', duration: 'Weeks 5–14', icon: Cog, items: ['Platform setup & scheduling config', 'Mobile deployment & device rollout', 'Route optimization & GIS integration'] },
  { phase: 'Launch', duration: 'Weeks 15–20', icon: Rocket, items: ['Pilot rollout & technician training', 'KPI monitoring & adjustment', 'Feedback collection & iteration'] },
  { phase: 'Scale', duration: 'Weeks 21–26', icon: TrendingUp, items: ['Full deployment across all regions', 'Optimization cycles & refinement', 'Continuous improvement program'] },
];

const caseStudies = [
  { client: 'Global Telecom Provider', industry: 'Telecom', timeline: '22 weeks', challenge: '10,000+ technicians with manual scheduling. Inconsistent service levels. Customers waiting days for simple repairs.', solution: 'ServiceNow FSM with Dynamic Scheduling and Mobile Agent across the entire technician workforce.', results: ['85% first-call resolution', '60% productivity improvement', '10K technicians managed', '40% less travel time'], quote: '10,000 technicians, one schedule. First-call resolution jumped to 85% and customers finally stopped waiting.', author: 'VP of Field Operations' },
];
...
const faqs = [
  ...
  { q: 'How long does implementation take?', a: '14–26 weeks depending on fleet size. Mobile deployment and scheduling configured in parallel. Pilot rollout typically by week 15.' },
...
export default function FSMPage() {
  ...
  return (
    <>
      ...
      <section className="relative bg-white overflow-hidden flex items-center min-h-[calc(100vh-70px)]">
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto py-12 sm:py-14">
          <div className="max-w-3xl">
            <div className="flex items-center gap-3 mb-8"><span className="inline-block w-8 h-px" style={{ backgroundColor: A }} /><span className="text-sm font-semibold tracking-widest uppercase" style={{ color: A }}>CRM & Customer</span></div>
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold text-gray-900 leading-[1.06] tracking-tight mb-6">Right technician.<br /><span style={{ color: A }}>Right place. Right time.</span></h1>
            <p className="text-lg text-gray-500 leading-relaxed mb-10 max-w-xl">AI-powered scheduling. Mobile-first operations. <span className="text-gray-900 font-semibold">70% scheduling efficiency. 60% productivity improvement.</span> Every truck, every tech, every job — optimized.</p>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Link href="/company/case-studies-client-success" className="inline-flex items-center justify-center gap-2 px-7 py-3.5 border border-gray-200 hover:border-cyan-400 text-gray-600 hover:text-cyan-600 font-semibold rounded-xl transition-colors text-sm">See Client Results</Link>
            </div>
          </div>
        </div>
      </section>
      <section className="py-16 lg:py-20 relative overflow-hidden" style={{ background: '#ecfeff' }}>
        ...
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto relative z-10">
          ...
          <div data-reveal="capabilities" className={`grid sm:grid-cols-2 lg:grid-cols-3 gap-5 transition-all duration-700 ${revealed.capabilities ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            {capabilities.map(({ icon: Icon, title, desc, outcomes }, i) => (
              <div key={title} className="group p-6 rounded-2xl border border-cyan-200 bg-white transition-all hover:shadow-md" style={{ transitionDelay: `${i * 80}ms` }}>
      ...
      <section className="py-16 lg:py-20 bg-white">
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
          ...
          <div className="grid md:grid-cols-4 gap-6 relative">
            <div className="hidden md:block absolute top-10 left-0 w-full h-px bg-gradient-to-r from-transparent via-cyan-200 to-transparent z-0" />
            {methodology.map(({ phase, duration, icon: Icon, items }) => (
              <div key={phase} className="relative z-10 group text-center">
                <div className="w-14 h-14 mx-auto mb-3 rounded-2xl bg-white border-2 border-cyan-100 group-hover:border-cyan-300 flex items-center justify-center transition-all shadow-sm group-hover:shadow-md"><Icon className="h-6 w-6" style={{ color: A }} /></div>
      ...
      <section className="py-16 lg:py-20" style={{ background: '#ecfeff' }}>
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
      ...
      <section className="py-16 lg:py-20 relative overflow-hidden" style={{ background: '#07071a' }}>
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto"><div className="max-w-4xl mx-auto">
          ...
          <div data-reveal="whyifbash" className={`grid sm:grid-cols-3 gap-5 max-w-3xl mx-auto transition-all duration-700 ${revealed.whyifbash ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            {[{ icon: ClipboardCheck, stat: 'Process first', desc: 'We ride along with your technicians before configuring a single schedule.' },{ icon: Clock, stat: 'Speed + quality', desc: 'Mobile deployment in weeks. Scheduling optimization visible within the first month.' },{ icon: Shield, stat: 'We stay', desc: '90-day hypercare minimum. We tune schedules until every technician\'s day flows.' }].map(({ icon: Icon, stat, desc }) => (<div key={stat} className="rounded-2xl border p-6 text-center transition-all" style={{ borderColor: `${A}26`, background: `${A}0A` }}><div className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-4" style={{ background: `${A}18` }}><Icon className="h-5 w-5" style={{ color: A }} /></div><div className="text-sm font-bold text-white mb-2">{stat}</div><p className="text-xs text-slate-400 leading-relaxed">{desc}</p></div>))}</div>
        </div></div>
      </section>
      <section className="py-16 lg:py-20 bg-white">
        <div className="w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto">
      ...
      <section className="py-16 relative overflow-hidden" style={{ background: '#07071a' }}>
        ...
        <div className="relative z-10 w-full px-4 sm:w-[95%] md:w-[90%] lg:w-[85%] xl:w-[80%] mx-auto text-center">
          <div className="max-w-xl mx-auto rounded-3xl border p-8 sm:p-10 text-center" style={{ borderColor: `${A}4d`, background: `linear-gradient(135deg, ${A}14, ${A}07)` }}>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-white leading-tight mb-3">Ready to optimize your field operations?</h2>
            <p className="text-slate-400 text-sm max-w-sm mx-auto mb-6">Tell us about your field workforce. We&apos;ll scope your FSM implementation in 48 hours.</p>
